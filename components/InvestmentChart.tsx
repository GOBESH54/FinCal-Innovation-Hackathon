...
function InvestmentChart({ breakdown }: InvestmentChartProps) {
  ...
  const chartSummary = useMemo(() => {
    if (chartData.length === 0) {
      return "No investment growth data is available yet.";
    }

    const firstPoint = chartData[0];
    const lastPoint = chartData[chartData.length - 1];

    return `By ${lastPoint.year}, the projected portfolio value grows to ${formatInrNumber(
      lastPoint.total
    )}, compared with ${formatInrNumber(lastPoint.invested)} invested and ${formatInrNumber(
      lastPoint.returns
    )} generated as returns. The chart begins at ${formatInrNumber(firstPoint.total)} in ${firstPoint.year}.`;
  }, [chartData]);
    ...
    <section className="app-card h-full min-w-0" aria-labelledby="chart-title">
      ...
      {chartData.length === 0 ? (
        ...
        <div className="space-y-3">
          ...
          <p className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm leading-6 text-slate-600 dark:border-slate-700 dark:bg-slate-800/80 dark:text-slate-300">
            {chartSummary}
          </p>
        </div>
      )}
